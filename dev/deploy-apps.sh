...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
Usage: deploy-apps.sh [--namespace apps-dev] [--acr-login-server <ACR_LOGIN_SERVER>]
...
if [[ -z "$acr_login_server" ]]; then
  echo "Set ACR_LOGIN_SERVER, pass --acr-login-server, or create dev/.env.local from .env.example." >&2
  exit 1
fi

cat "$script_dir/otelapidemo-dotnet.yaml" "$script_dir/otelapidemo-python.yaml" "$script_dir/otelapidemo-cpp.yaml" "$script_dir/otel-ui.yaml" "$script_dir/otelapidemo-ingress.yaml" "$script_dir/otel-ui-ingress.yaml" "$script_dir/otel-ui-otlp-service.yaml" "$script_dir/otel-ui-otlp-ingress.yaml" |
